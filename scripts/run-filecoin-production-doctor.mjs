...
import { buildFilecoinProductionDoctorReport } from "../src/filecoinProductionDoctor.ts";
...
const report = await buildFilecoinProductionDoctorReport(env);

if (process.argv.includes("--json")) {
  console.log(JSON.stringify({ envFiles: loaded, ...report }, null, 2));
} else {
  console.log("Filecoin production doctor");
